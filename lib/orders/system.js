...
var dockerCpuUsage = function() {
  ...
  // /proc/<pid>/stat stores process use cpu time counted in clock ticks
  // sysconf(_SC_CLK_TCK) returns the system parameter, default value 100
  // which means 100 ticks per second, so the following *10 returns process
  // cpu useage counted in ms
  var diff_used = (now_used - last_used) * 10;
  var diff_sys = now_sys - last_sys;
  last_used = now_used;
  last_sys = now_sys;
  // docker cpu useage percent %
  return diff_used / diff_sys / CPU_COUNT;
};
...
exports.init = function () {
  IS_DOCKER = _isInDocker();
  IS_LINUX = os.type() === 'Linux';
  if (IS_DOCKER) {
    CPU_COUNT = _getDockerCPUs();
  }
  last_used = IS_DOCKER ? _getAllUsedCpu() : 0;
  last_sys = IS_DOCKER ? new Date().getTime(): 0;
};
